...
class SceneView(openglGui.glGuiPanel):
	...
	def _drawMachine(self):
		...
		if machine.startswith('creatableD2'):			
			if machine not in self._platformMesh:
				

				meshes = meshLoader.loadMeshes(resources.getPathForMesh('creatableD2_platform.stl'))
				if len(meshes) > 0:
				...
				self._platformMesh[machine]._drawOffset = numpy.array([0,0,0], numpy.float32)			
			...
			# glRotate(120,0,0,1)
			# glRotate(90,1,0,0)
			# glTranslate(-120,-120,-7)
			glTranslate(-120,-120,-6.2)
			...
			if not hasattr(self._platformMesh[machine], 'texture'):
				...
				self._platformMesh[machine].texture = openglHelpers.loadGLTexture('Bed_MappingText.png')
			...
			if machine not in self._platformMesh:
				...
				meshes = meshLoader.loadMeshes(resources.getPathForMesh('creatableD3_platform.stl'))
				if len(meshes) > 0:
					self._platformMesh[machine] = meshes[0]
				else:
					self._platformMesh[machine] = None
		...
		if self._platformTexture is None:
			self._platformTexture = openglHelpers.loadGLTexture('Grid.png')
